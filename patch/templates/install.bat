@echo off
rem ===========================================================================
rem  NPC Friends x Heap of Foods - cooking patch : INSTALL
rem
...
chcp 65001 >nul 2>&1
title NPC Friends x Heap of Foods - Cooking Patch (install)

if not exist "%~dp0tools\patch.ps1" (
  echo.
  echo  [ERROR] tools\patch.ps1 not found.
  echo          Unzip the whole package first, then run install.bat
...
powershell -NoProfile -ExecutionPolicy Bypass -File "%~dp0tools\patch.ps1" -Action install
set "RC=%ERRORLEVEL%"
